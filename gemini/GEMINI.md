# Working with the TIA Portal Automation Harness (Gemini CLI)

> **Sync note:** This file is a port of [`.claude-plugin/CLAUDE.md`](../.claude-plugin/CLAUDE.md)
> for **Gemini CLI**. The Claude Code plugin is the **source of truth** — keep the technical
> content (gates, guards, tool map, gotchas) in sync with it. Only the vocabulary/format is
> adapted here: subagents live in `.gemini/agents/`, commands in `.gemini/commands/`, and the
> `tia` MCP server is wired via `~/.gemini/settings.json` (see [`docs/INSTALL.md`](../docs/INSTALL.md)).

## What this harness is

This project uses the **TIA Portal Automation Harness** — a Gemini CLI extension that drives
**Siemens TIA Portal V21** via the **`tia` MCP** over the project's Openness HTTP `serve`. It
turns a structured engineering **project-spec** into a *compliant-by-construction*, *verified*
TIA project. The harness ships commands (in `.gemini/commands/`) — `spec.validate`, `cpu.select`,
`spec.plan`, `tia.review`, `tia.scaffold`, `tia.verify`, `tia.handoff` — plus subagents (in
`.gemini/agents/`: tia-engineer, hardware-architect, verifier, safety-reviewer). The model is an
**assistant up to the gate, never an autonomous commissioner**.
...
- The `tia` MCP server must be registered in **`~/.gemini/settings.json`** (under `mcpServers`)
  — see [`docs/INSTALL.md`](../docs/INSTALL.md) for the exact stanza and how Gemini CLI discovers it.
...
are a *different* class — see the **`tia.handoff`** command. Those are "a human **approves an action**",
