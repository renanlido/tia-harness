> **Sync note:** The block between the `tia-harness` markers below is GENERATED from
> `mcp/content/` (run `cd mcp && npm run sync`) — do not hand-edit inside it. This head
> (outside the markers) is Gemini-port-specific: subagents live in `.gemini/agents/`,
> commands in `.gemini/commands/`, and the `tia` MCP server is wired via
> `~/.gemini/settings.json` (see [`docs/INSTALL.md`](../docs/INSTALL.md)).
