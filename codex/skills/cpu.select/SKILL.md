...
description: >-
  ...
  an engineering need (I/O count, scan/performance, memory, communication ports, safety,
  redundancy, motion). USE when the user asks "which CPU / PLC should I use", gives
  selection criteria instead of a model, or needs a typeIdentifier for a project-spec.
  Output: recommended CPU + justification + the OrderNumber:<MLFB>/V<fw> TypeIdentifier
  (flagged: confirm/resolve the concrete MLFB+firmware via GET /catalog before building).
  Do NOT use when the user already gave a concrete model, to validate a spec (use
  spec.validate), to build a plan (use spec.plan), or to audit a project (use tia.review).
  Deterministic heuristic over the bundled CPU reference — NO MCP.
---
