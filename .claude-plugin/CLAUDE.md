...
This project uses the **TIA Portal Automation Harness** — a Claude Code plugin that drives
...
TIA project. The harness ships skills — `spec.validate`, `cpu.select`, `spec.plan`,
...
Node backend → protocol → PLC). See the `tia-openness` project's `docs/COMM-SCENARIOS.md` and
`comm-scenarios/` (`modbus/`, `s7/`,
`opcua/`).
